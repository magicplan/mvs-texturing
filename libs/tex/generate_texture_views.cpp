...
void from_images_and_camera_files(std::string const &path,
  ...
  for (std::size_t i = 0; i < dir.size(); ++i) {
    ...
    for (std::size_t j = i + 1;; j += step) {
      /* Since the files are sorted we can break - no more files with the same
       * prefix exist. */
      if (j >= dir.size() ||
  ...
  tbb::parallel_for(
      std::size_t(0), files.size() / 2, [&](const std::size_t i_div_2) {
        ...
        if (cam_info.dist[0] != 0.0f) {
          ...
          if (cam_info.dist[1] != 0.0f) {
            ...
            image = mve::image::image_undistort_vsfm<uint8_t>(
                image, cam_info.flen, cam_info.dist[0]);
          }
