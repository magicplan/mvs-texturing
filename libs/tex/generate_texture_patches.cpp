...
template <typename T> T clamp_nan_low(T const &v, T const &lo, T const &hi) {
  return (v > lo) ? ((v < hi) ? v : hi) : lo;
}

template <typename T> T clamp_nan_hi(T const &v, T const &lo, T const &hi) {
  return (v < hi) ? ((v > lo) ? v : lo) : hi;
}

template <typename T> T clamp(T const &v, T const &lo, T const &hi) {
...
void merge_vertex_projection_infos(
  ...
  tbb::parallel_for(
      std::size_t(0), vertex_projection_infos->size(),
      [&](const std::size_t i) {
        std::vector<VertexProjectionInfo> &infos =
            vertex_projection_infos->at(i);
...
void generate_texture_patches(
  ...
  tbb::parallel_for(
      std::size_t(0), texture_views->size(), [&](const std::size_t i) {
          ...
          for (std::size_t i = 0; i < faces.size(); ++i) {
            ...
            for (std::size_t j = 0; j < 3; ++j) {
              ...
              VertexProjectionInfo info = {
                  texture_patch_id, projection, {face_id}};
...

  {
    ...
    tbb::parallel_for(
        std::size_t(0), subgraphs.size(), [&](const std::size_t i) {
