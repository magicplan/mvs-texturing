...
void calculate_face_projection_infos(
  ...
  tbb::parallel_for(
      std::uint16_t(0), static_cast<std::uint16_t>(num_views),
      [&](const std::uint16_t j) {
        {
          ...
          for (std::size_t i = 0; i < faces.size(); i += 3) {
            ...
            math::Vec3f view_to_face_vec =
                (face_center - view_pos).normalized();
            math::Vec3f face_to_view_vec =
                (view_pos - face_center).normalized();
