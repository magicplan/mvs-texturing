...
calculate_difference(VertexProjectionInfos const &vertex_projection_infos,
                     ...
                     std::vector<MeshEdge> const &seam_edges, int label1,
                     int label2) {
  assert(label1 != 0 && label2 != 0 && label1 < label2);
...
void global_seam_leveling(UniGraph const &graph,
  ...
  /* Assign each vertex for each label a new index(row) within the solution
   * vector x. */
  std::size_t x_row = 0;
  ...
  /* Fill the matrix A and the coefficients for the Vector b of the linear
   * equation system. */
  ...
  /* Only keep lower triangle (CG only uses the lower), prune the rest and
   * compress matrix. */
  ...
  tbb::parallel_for(std::size_t(0), std::size_t(3), [&](std::size_t channel) {
    ...
    /* Subtract mean because system is underconstrained and we seek the solution
  ...
  tbb::parallel_for(
      std::size_t(0), texture_patches->size(), [&](std::size_t i) {
